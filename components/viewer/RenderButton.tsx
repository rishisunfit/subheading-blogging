export function RenderButton({ attrs }: { attrs: any }) {
  const { text, url, variant, color, customColor, customColorSecondary, useGradient, gradientDirection, size, radius, align } = attrs;
  ...
  // Color and variant combinations (same as ButtonComponent)
  const getButtonClasses = () => {
    ...
    if (color === "custom") {
      const base = `font-medium transition-all duration-200 inline-flex items-center justify-center cursor-pointer ${sizeClasses[size as keyof typeof sizeClasses] || sizeClasses.md} ${radiusClasses[radius as keyof typeof radiusClasses] || radiusClasses.md}`;
    ...
    return `${baseClasses} ${sizeClasses[size as keyof typeof sizeClasses] || sizeClasses.md} ${radiusClasses[radius as keyof typeof radiusClasses] || radiusClasses.md} ${variantColorClasses[variant]?.[color] || variantColorClasses.solid.primary}`;
  };
  ...
  return (
    <div
      className={`my-4 ${alignmentClasses[align as keyof typeof alignmentClasses] || alignmentClasses.center}`}
    >
      <a
        href={url}
        target="_blank"
